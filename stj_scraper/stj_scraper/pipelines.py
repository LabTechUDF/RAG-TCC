...
class ValidationPipeline:
    """Validate scraped STJ legal documents and assess content quality"""
    ...
    def calculate_content_quality(self, adapter):
        ...
        # URL quality (25 points) - Must be scon.stj.jus.br
        url = adapter.get('url', '')
        ...
        # Title quality (25 points) - Must contain STJ legal decision acronyms
        title = adapter.get('title', '')
        legal_acronyms = ['ARESP', 'RESP', 'RHC', 'HC', 'MS', 'MC', 'AgRg']
        if title:
        ...
        raw_text = adapter.get('raw_text', '') or adapter.get('content', '')
        if raw_text:
            content_clean = raw_text.strip()
            # Check for meaningful content length and STJ-specific structure
            if len(content_clean) > 500 and ('Relator' in content_clean or 'RELATORA' in content_clean):
                score += 25
            elif len(content_clean) > 200 and ('STJ' in content_clean or 'SUPERIOR TRIBUNAL' in content_clean):
                score += 20
...
class StatisticsPipeline:
    """Collect statistics about scraped STJ items"""
