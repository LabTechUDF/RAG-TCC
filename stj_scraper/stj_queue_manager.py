"""
STJ Query Queue Manager - Handles thread-safe processing of queries for STJ SCON
"""
import json
import subprocess
import time
...
class STJQueryQueue:
    """
    Thread-safe queue-based manager for STJ queries:
    ...
    def __init__(self, project_root: Path):
        ...
        self.queue_file = project_root / "stj_scraper" / "queue_state.json"
        self.lock_file = project_root / "stj_scraper" / "queue.lock"
        self.current_query: Optional[Dict] = None
        self.thread_local = threading.local()  # Thread-local storage for locks
        
        # Setup logging
        self.logger = logging.getLogger('STJQueryQueue')
    ...
    def get_next_query(self) -> Optional[Dict]:
        ...
        try:
            ...
            if not state['queue']:
                self.logger.info("No more queries in STJ queue")
                return None
            ...
            self.logger.info(f"🎯 Retrieved Article {next_query['artigo']} from STJ queue")
            self.logger.info(f"📊 Remaining STJ queries: {len(state['queue'])}")
    ...
    def mark_query_completed(self, query: Dict, success: bool, error: Optional[str] = None):
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"✅ STJ Article {query['artigo']}: Marked as completed")
            else:
                ...
                self.logger.error(f"❌ STJ Article {query['artigo']}: Marked as failed")
    ...
    def load_queries(self, query_file_path: Path) -> bool:
        ...
        try:
            ...
            self.logger.info(f"📋 Loaded {len(queries)} queries into thread-safe STJ queue")
            
            # Log queue contents
            for i, query in enumerate(queries):
                article = query.get('artigo', f'query_{i}')
                self.logger.info(f"   {i+1}. STJ Article {article}")
            
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to load STJ queries: {e}")
            return False
    ...
    def create_single_query_file(self, query: Dict, temp_file_path: Path) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Created single STJ query file: {temp_file_path}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error creating single STJ query file: {e}")
            return False
    
    def run_single_spider(self, query: Dict, show_browser: bool = False) -> bool:
        """Run STJ spider with a single query"""
        article = query['artigo']
        
        # Create temporary query file
        temp_dir = self.project_root / 'temp_queue'
        temp_file = temp_dir / f'stj_query_{article}_{int(time.time())}.json'
        
        try:
            self.logger.info(f"🚀 Starting STJ spider for Article {article}")
            
            if not self.create_single_query_file(query, temp_file):
                return False
            
            # Build scrapy command for STJ
            cmd = [
            ...
            # Run the command
            self.logger.info(f"📋 Executing STJ command: {' '.join(cmd)}")
            ...
            if result.returncode == 0:
                self.logger.info(f"✅ STJ Article {article}: Spider completed successfully")
                self.mark_query_completed(query, success=True)
                return True
            else:
                self.logger.error(f"❌ STJ Article {article}: Spider failed")
            ...
            self.logger.error(f"⏰ STJ Article {article}: Spider timed out after 30 minutes")
            self.mark_query_completed(query, success=False, error='Timeout after 30 minutes')
            return False
            
        except Exception as e:
            self.logger.error(f"💥 STJ Article {article}: Exception occurred: {e}")
            ...
            try:
                if temp_file.exists():
                    temp_file.unlink()
                    self.logger.debug(f"Cleaned up STJ temp file: {temp_file}")
            except Exception as e:
                self.logger.warning(f"Failed to clean up STJ temp file: {e}")
    
    def process_queue(self, show_browser: bool = False) -> Dict:
        """Process all STJ queries in the queue sequentially (thread-safe version)"""
        start_time = datetime.now()
        
        self.logger.info("🎯 Starting sequential STJ query processing")
        self.logger.info("=" * 60)
        ...
        while True:
            ...
            self.logger.info(f"📋 Processing STJ Article {article} ({processed_count}/{total_queries})")
            
            # Run spider for this single query
            success = self.run_single_spider(query, show_browser)
            
            if success:
                self.logger.info(f"✅ STJ Article {article}: Processing completed")
            else:
                self.logger.error(f"❌ STJ Article {article}: Processing failed")
    ...
    def process_single_query(self, show_browser: bool = False) -> Optional[Dict]:
        """Process a single STJ query from the queue (for concurrent processing)"""
        query = self.get_next_query()
        if query is None:
            return None
        
        article = query['artigo']
        self.logger.info(f"🎯 STJ Worker processing Article {article}")
        ...
        if success:
            self.logger.info(f"✅ STJ Worker completed Article {article}")
        else:
            self.logger.error(f"❌ STJ Worker failed Article {article}")
        
        return result
    
    def get_queue_status(self) -> Dict:
        """Get current STJ queue status"""
        if not self._acquire_lock():
            self.logger.error("Failed to acquire lock for getting STJ queue status")
            return {}
    ...
    def cleanup_queue_files(self):
        """Clean up STJ queue state and lock files"""
        try:
            if self.queue_file.exists():
                self.queue_file.unlink()
                self.logger.info("Cleaned up STJ queue state file")
            
            if self.lock_file.exists():
                self.lock_file.unlink()
                self.logger.info("Cleaned up STJ lock file")
                
        except Exception as e:
            self.logger.error(f"Failed to cleanup STJ queue files: {e}")
    
    def print_final_report(self, report: Dict):
        """Print a comprehensive final report for STJ processing"""
        self.logger.info("")
        self.logger.info("=" * 60)
        self.logger.info("📊 FINAL STJ PROCESSING REPORT")
        self.logger.info("=" * 60)
        
        self.logger.info(f"⏱️  Total duration: {report['duration']}")
        self.logger.info(f"📋 Total STJ queries: {report['total_queries']}")
        ...
        if report['completed_queries']:
            self.logger.info("\n✅ Successfully processed STJ articles:")
            for item in report['completed_queries']:
                article = item['query']['artigo']
                self.logger.info(f"   • STJ Article {article}")
        ...
        if report['failed_queries']:
            ...
            for item in report['failed_queries']:
                ...
                self.logger.info(f"   • STJ Article {article}: {error}")
        
        self.logger.info("=" * 60)
    
    def count_extracted_items(self) -> Dict:
        """Count items extracted for each successfully processed STJ article"""
        main_data_dir = self.project_root / 'data' / 'stj_jurisprudencia'
        
        results = {}
        total_items = 0
        
        if not main_data_dir.exists():
            self.logger.warning("No STJ data directory found")
            return results
        ...
        for json_file in main_data_dir.rglob('*.jsonl'):
            try:
                ...
                self.logger.warning(f"Error counting STJ items in {json_file}: {e}")
        
        self.logger.info(f"\n📊 Extracted STJ items summary:")
        for article, count in sorted(results.items()):
            self.logger.info(f"   STJ Article {article}: {count} items")
        self.logger.info(f"   Total STJ: {total_items} items")
        
        return results


def run_stj_queue_based(project_root: Path, query_file: Path, show_browser: bool = False) -> Dict:
    """
    Main function to run STJ scraping with queue-based architecture
    """
    queue_manager = STJQueryQueue(project_root)
    
    # Load queries into queue
    if not queue_manager.load_queries(query_file):
        return {'error': 'Failed to load STJ queries'}
...
if __name__ == '__main__':
    # Test the STJ queue system
    project_root = Path(__file__).parent
    query_file = project_root / 'test_queries.json'
    
    print("🧪 Testing STJ Queue System")
    report = run_stj_queue_based(project_root, query_file)
    print(f"STJ test completed: {report['successful']}/{report['total_queries']} successful")