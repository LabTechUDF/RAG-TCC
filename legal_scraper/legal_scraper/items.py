...
class SumulaItem(LegalDocumentItem):
    """Item for Súmulas (legal precedents)"""
    
    # Súmula specifics
    sumula_number = scrapy.Field(
        output_processor=TakeFirst()
    )
    
    sumula_type = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    binding_effect = scrapy.Field(
        output_processor=TakeFirst()
    )
...
class NormativaItem(LegalDocumentItem):
    """Item for normative acts"""
    
    # Normative specifics
    normative_type = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    normative_number = scrapy.Field(
        output_processor=TakeFirst()
    )
    ...
    revoked = scrapy.Field(
        output_processor=TakeFirst()
    )
    
    related_laws = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=list
    )


class DireitoPenalItem(LegalDocumentItem):
    """Item for criminal law content"""
    ...
    penalty_type = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    legal_provision = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    precedent_references = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=list
    )


class TribunalEstadualItem(LegalDocumentItem):
    """Item for state court decisions"""
    
    # State court specifics
    state = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    court_chamber = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
    
    instance_level = scrapy.Field(
        input_processor=MapCompose(clean_text),
        output_processor=TakeFirst()
    )
