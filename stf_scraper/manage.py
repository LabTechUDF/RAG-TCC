#!/usr/bin/env python3
"""
STF Queue-Based Scraper Manager
"""
...
class STFQueueManager:
    ...
    def run_stf_sequential_queue(self, show_browser=False):
        print("🎯 Running STF Jurisprudencia with Sequential Queue Architecture")
        ...
        try:
            report = run_stf_queue_based(self.project_root, query_file, show_browser)
    ...
    def run_stf_concurrent_queue(self, max_workers=3, show_browser=False):
...
def main():
    parser = argparse.ArgumentParser(description='STF Queue-Based Scraper Manager')
    ...
    try:
        if args.command == 'sequential':
            success = manager.run_stf_sequential_queue(show_browser=args.show_browser)
